  ...
  var QueryGenerator = {
    ...
    */
    ...
    selectQuery: function(tableName, options, factory) {
      ...
      if (options.order) {
        console.log('~~~~~~~~~~~WWW1~~~', options.order);
        if (typeof(options.order) === 'string') {
          if (options.order.indexOf(',') >= 0) {
            options.order = options.order.split(/[,]+/);
          } else {
            options.order = [ options.order.trim() ];
          }
        }
        console.log('~~~~~~~~~~~WWW2~~~', options.order);
        options.order = options.order.map(function (t) {
          var element = t.trim().split(/[ ]+/);
          element[0] = this.quote(element[0]);
          return element.join(' ');
        }.bind(this)).join(', ')

        console.log('~~~~~~~~~~~WWW~~~', options.order);
        if (subQuery) {
